...
import { IBrand, IBrandApiResponse } from "@/types/interface";
...
const BrandPage = () => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      const res = await apiService.delete(`/brand/${categoryId}`, {
        withAuth: true,
      });

      if (res.status === 200) {
        toast.success("Brand deleted successfully");
        const updated = brand.filter((item) => item.id !== categoryId);
  ...
  const fetchCategories = useCallback(
    async (sortKey = "", sortOrder = "") => {
      try {
        ...
        const res: IBrandApiResponse = await apiService.get(
          `/brand?page=${page}&pageSize=${pageSize}&search=${searchTerm}&sortBy=${sortKey}&order=${sortOrder}`,
          { withAuth: true },
        );
  ...
  const handleSort = (key: keyof IBrand) => {
  ...
  const handleCreate = () => router.push("/brand/new");

  const handleEdit = (id: number) => router.push(`/brand/${id}`);
