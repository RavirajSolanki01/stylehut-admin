...
const ProductDetailKeyPage = () => {
  ...
  const handleDeleteConfirm = async () => {
    try {
      const res = await apiService.delete(
        `/product/additional-details-key/${categoryId}`,
        {
          withAuth: true,
        },
      );
    ...
    async (sortKey = "", sortOrder = "") => {
      try {
        ...
        const res: IProductDetailKeyApiResponse = await apiService.get(
          `/product/additional-details-key?page=${page}&pageSize=${pageSize}&search=${searchTerm}&sortBy=${sortKey}&order=${sortOrder}`,
          { withAuth: true },
        );
  ...
  const handleCreate = () => router.push("/product-detail-key/new");

  const handleEdit = (id: number) => router.push(`/product-detail-key/${id}`);
  ...
  return (
    <>
      <Head>
        <title>Product Detail Key</title>
      </Head>
...
export default ProductDetailKeyPage;
