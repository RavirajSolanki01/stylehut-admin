...
import { IProduct, IProductApiResponse } from "@/types/interface";
...
const ProductPage = () => {
  ...
  const [product, setProductTypes] = useState<IProduct[]>([]);
  const [pagination, setPagination] = useState(DEFAULT_PAGINATION);
  const [sortKey, setSortKey] = useState<keyof IProduct | "">("");
  const [sortOrder, setSortOrder] = useState<"asc" | "desc">("asc");
  const [isLoading, setIsLoading] = useState(false);

  const [isDialogOpen, setIsDialogOpen] = useState(false);
  const [productId, setCategoryTypeId] = useState<number>(0);

  const handleDeleteConfirm = async () => {
    try {
      const res = await apiService.delete(`/product/${productId}`, {
        withAuth: true,
      });

      if (res.status === 200) {
        toast.success("Product type deleted successfully");
        const updated = product.filter((item) => item.id !== productId);
  ...
  const fetchCategories = useCallback(
    async (sortKey = "", sortOrder = "") => {
      try {
        ...
        const res: IProductApiResponse = await apiService.get(
          `/product?page=${page}&pageSize=${pageSize}&search=${searchTerm}&sortBy=${sortKey}&order=${sortOrder}`,
          { withAuth: true },
        );
  ...
  const handleCreate = () => router.push("/product/new");
          ...
          <DynamicTable
            ...
            columns={[
              ...
              },
              {
                ...
                label: "Image",
                render: (_value, row) => (
                  <ImageHoverPreview images={row.image || []} />
                ),
              },
              {
                key: "create_at",
