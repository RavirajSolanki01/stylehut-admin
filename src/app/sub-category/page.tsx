...
import { ISubCategory, ISubCategoryApiResponse } from "@/types/interface";
...
const CategoryPage = () => {
  ...
  const [sortKey, setSortKey] = useState<keyof ISubCategory | "">("");
  ...
  const handleDeleteConfirm = async () => {
    try {
      const res = await apiService.delete(`/sub-category/${subCategoryId}`, {
        withAuth: true,
      });
  ...
  const fetchCategories = useCallback(
    async (sortKey = "", sortOrder = "") => {
      try {
        ...
        const res: ISubCategoryApiResponse = await apiService.get(
          `/sub-category?page=${page}&pageSize=${pageSize}&search=${searchTerm}&sortBy=${sortKey}&order=${sortOrder}`,
          { withAuth: true },
        );
  ...
  const handleSort = (key: keyof ISubCategory) => {
  ...
  const handleCreate = () => router.push("/sub-category/new");

  const handleEdit = (id: number) => router.push(`/sub-category/${id}`);
      ...
      <Layout>
        ...
        <div className="space-y-10">
          ...
          <CommonDialog
            ...
            title="Subcategory"
            description="Are you sure you want to delete subcategory?"
          />
        </div>
      </Layout>
    </>
