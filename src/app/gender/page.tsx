...
import { IGender, IGenderApiResponse } from "@/types/interface";
...
const GenderPage = () => {
  ...
  const [sortKey, setSortKey] = useState<keyof IGender | "">("");
  ...
  const handleDeleteConfirm = async () => {
    try {
      const res = await apiService.delete(`/gender/${categoryId}`, {
        withAuth: true,
      });

      if (res.status === 200) {
        toast.success("Gender deleted successfully");
        const updated = gender.filter((item) => item.id !== categoryId);
  ...
  const handleSort = (key: keyof IGender) => {
  ...
  const handleCreate = () => router.push("/gender/new");

  const handleEdit = (id: number) => router.push(`/gender/${id}`);
      ...
      <Layout>
        ...
        <div className="space-y-10">
          ...
          <CommonDialog
            ...
            title="Gender"
            description="Are you sure you want to remove this gender?"
          />
        </div>
      </Layout>
    </>
